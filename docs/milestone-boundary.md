...
- Node-specific initializers now have an explicit registry. `PropertyNode` is the first initializer-backed graph-addable node, and `nodeCatalogClassification.initializerBackedNodeTypes` separates these promoted nodes from the plain parameterless probe path.
...
- Configurable graph-authoring nodes are classified under `nodeCatalogClassification.configurableNodeClassification`. The current supported path keeps initializer-backed `PropertyNode` separate through explicit property binding metadata; `CustomFunction`, `Dropdown`, and `Keyword` remain diagnostic-only until explicit configuration serialization is implemented, and arbitrary `SubGraphNode` composition remains diagnostic-only until explicit `.shadersubgraph` asset binding is implemented.
