...
- `connect_ports` now also supports the first package-backed logic path: `Vector1` or scalar arithmetic outputs into `ComparisonNode.A/B`, `ComparisonNode.Out -> BranchNode.Predicate`, `Vector1` or scalar arithmetic outputs into `BranchNode.True/False`, and `BranchNode.Out -> Vector1Node.X` or scalar arithmetic inputs.
