...
This repository is currently in the `1.4.0` explicit configurable and asset-bound node promotion milestone.
...
- `import_graph_contract` is now documented as requiring a blank-or-near-blank target graph before replaying an exported contract.
...
- The current EditMode smoke coverage now also locks direct `UVNode.Out -> SampleTexture2DNode.UV` routing on a blank graph.
- The current EditMode smoke coverage now also locks graph/subgraph contract mismatch import failures for `import_graph_contract` across real graph and subgraph fixtures.
...
- `connect_ports` currently supports the verified narrow rules plus the current scalar, vector-builder, logic, early Color-routing, and UV/texture-derived normal input package-backed paths: `Vector1Node.Out -> Vector1Node.X`, `ColorNode.Out -> SplitNode.In`, `UVNode.Out -> TilingAndOffsetNode.UV/SampleTexture2DNode.UV`, `TilingAndOffsetNode.Out -> SampleTexture2DNode.UV`, `SampleTexture2DNode.R/G/B/A -> ComparisonNode.A/B`, `SampleTexture2DNode.R/G/B/A -> BranchNode.True/False`, `SplitNode.R/G/B/A -> Vector1Node.X`, `Vector1/Split/arithmetic/Dropdown/Keyword/CustomFunction -> CombineNode.R/G/B/A`, `Vector1/Split/arithmetic/Dropdown/Keyword/CustomFunction -> Vector2/Vector3/Vector4` scalar inputs, `CombineNode.RGBA -> SplitNode.In`, `Vector4Node.Out -> SplitNode.In`, `Vector1Node.Out -> Add/Subtract/Multiply/Divide/Power/Minimum/Maximum/Modulo/Lerp/Smoothstep/Clamp/Step/Absolute/Floor/Ceiling/Round/Sign/Sine/Cosine/Tangent/Negate/Reciprocal/SquareRoot/Fraction/Truncate/Saturate/OneMinus` inputs, arithmetic `Out -> Vector1Node.X`, arithmetic `Out -> arithmetic` inputs, `Vector1/arithmetic/Dropdown/Keyword/CustomFunction -> Comparison.A/B`, `Comparison.Out -> one or more Branch.Predicate` inputs, `Vector1/arithmetic/Dropdown/Keyword/CustomFunction -> Branch.True/False`, `Branch.Out -> one or more Vector1.X/arithmetic inputs`, `Color/Combine RGBA/Vector4/Multiply/Branch/Lerp/Append -> Multiply.A/B`, `Color/Combine RGBA/Vector4/Multiply/Branch/Lerp/Append -> Branch.True/False`, `Color/Combine RGBA/Vector4/Multiply/Branch/Lerp/Append -> Lerp.A/B/T`, `Color/Combine RGBA/Vector4/Multiply/Branch/Lerp/Append/SampleTexture2D.RGBA -> NormalBlend.A/B`, `Color/Combine RGBA/Vector4/Multiply/Branch/Lerp/Append plus Vector1/Split/arithmetic/SampleTexture2D channels/Dropdown/Keyword/CustomFunction -> Append.A/B`, `Texture2DAssetNode.Out -> NormalFromTextureNode.Texture`, `UVNode/TilingAndOffsetNode.Out -> NormalFromTextureNode.UV`, `Vector1/arithmetic/Dropdown/Keyword/CustomFunction -> NormalFromTextureNode.Offset/Strength`, `Multiply.Out -> SplitNode.In`, `Branch.Out -> SplitNode.In`, `Lerp.Out -> SplitNode.In`, and `AppendVectorNode.Out -> SplitNode.In`.
...
- Node-specific initializers now have an explicit registry. `PropertyNode`, `DropdownNode`, `KeywordNode`, string-body `CustomFunctionNode`, and explicit-asset `SubGraphNode` are initializer-backed graph-addable paths, and `nodeCatalogClassification.initializerBackedNodeTypes` separates these promoted nodes from the plain parameterless probe path.
- Texture/sample node promotion is classified under `nodeCatalogClassification.textureSampleNodeClassification`. The current fixture-backed add/save batch locks `Texture2DAsset`, `SampleTexture2D`, `Texture2DArrayAsset`, `SampleTexture2DArray`, `Texture3DAsset`, `SampleTexture3D`, `CubemapAsset`, `SampleCubemap`, and `SamplerState`; externally asset-bound candidates such as `SampleVirtualTexture` stay diagnostic-only unless the package probe proves them graph-addable.
- Coordinate, scene, camera, and utility node promotion is classified under `nodeCatalogClassification.coordinateUtilityNodeClassification`. The current add/save batch locks `UV`, `ScreenPosition`, `Position`, `NormalVector`, `TangentVector`, `BitangentVector`, `ViewDirection`, `Time`, `Object`, `Camera`, `Transform`, and `TransformationMatrix`; scene/camera or platform-context nodes that vary by Unity/Shader Graph version stay diagnostic-only until the probe proves them graph-addable.
- Normal, lighting, reflection, rendering, and material-helper node promotion is classified under `nodeCatalogClassification.normalLightingRenderingNodeClassification`. The current add/save batch locks `NormalBlend`, `NormalFromHeight`, `NormalFromTexture`, `NormalReconstructZ`, `NormalStrength`, `NormalUnpack`, `Ambient`, `BakedGI`, `Blackbody`, `DielectricSpecular`, `MainLightDirection`, `MetalReflectance`, `Reflection`, `ReflectionProbe`, `Fog`, `RenderType`, and `RenderTypeBranch`; render-pipeline-specific helpers remain diagnostic-only unless the package probe proves them graph-addable.
- The graph-addable promoted node batches above now also have graph contract replay coverage for `export_graph_contract -> import_graph_contract`, preserving node type, display name, and position for asset-independent nodes.
- Configurable graph-authoring nodes are classified under `nodeCatalogClassification.configurableNodeClassification`. The current supported path keeps initializer-backed `PropertyNode` separate through explicit property binding metadata; `Dropdown`, `Keyword`, and string-body `CustomFunction` nodes require explicit configuration metadata, and `SubGraphNode` requires explicit `.shadersubgraph` asset binding before graph-addable support is enabled.
- Specialized nodes are classified under `nodeCatalogClassification.specializedNodeClassification`. The current add/save batch locks portable default asset nodes `DefaultBitmapText`, `DefaultGradient`, `DefaultSDFText`, `DefaultSolid`, and `DefaultTexture`; package-context UI, sprite, VFX, deformation, and custom-interpolator nodes remain diagnostic-only until explicit package-context serialization is implemented.
