...
- Connection conversion, general fan-out beyond the current `Comparison.Out -> multiple Branch.Predicate` and `Branch.Out -> multiple Vector1/arithmetic inputs` paths, cross-type coercion, direct boolean-to-scalar routing outside `Branch.Predicate`, Color node inputs, and broader Color connections beyond the currently verified `Multiply`, `Branch`, `Lerp`, and `Append` vector paths stay unsupported.
