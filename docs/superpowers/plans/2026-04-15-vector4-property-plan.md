# Vector4 Property Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add package-backed `Vector4` Shader Graph property support with matching catalog and metadata coverage.
...
- [ ] Add a smoke test that requires `list_supported_properties` to include `Vector4`.
- [ ] Add a smoke test that adds a `Vector4` property to a blank graph and checks the resolved property type.
- [ ] Update metadata tests so the supported-property catalog and package-backed property envelopes preserve `Vector4`.

### Task 2: Implement package-backed Vector4 property support

**Files:**
- Modify: `packages/unity-shader-graph-mcp/Editor/Adapters/ShaderGraphPackageBackend.cs`

- [ ] Extend the supported property type list to include `Vector4`.
- [ ] Resolve `UnityEditor.ShaderGraph.Internal.Vector4ShaderProperty` in the property-type resolver.
- [ ] Teach property-instance inspection to map `Vector4ShaderProperty` back to canonical type `Vector4`.
- [ ] Add a minimal invariant-culture `Vector4` default-value parser and wire it into shader-input creation and update paths.
...
- [ ] Update the documented property boundary so it mentions `Vector4` alongside the previously shipped property types.
