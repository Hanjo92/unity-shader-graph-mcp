...
- For final `1.2.0`, run one real Unity batchmode bridge smoke with `UNITY_SHADER_GRAPH_MCP_UNITY_EXE` and `UNITY_SHADER_GRAPH_MCP_UNITY_PROJECT` set when a Unity project is available.
...
- Describe this cut as the focused post-`1.1` package-backed Shader Graph backlog release.
- Call out Boolean property-node predicate routing, `SampleGradient` graph-addable promotion, and graph/subgraph contract path parity.
...
- Use post-release feedback to choose the next post-`1.2` slice across graph-addable catalog expansion, deeper subgraph composition, and remaining verified connection routes.
