...
- For final `1.3.0`, run one real Unity batchmode bridge smoke with `UNITY_SHADER_GRAPH_MCP_UNITY_EXE` and `UNITY_SHADER_GRAPH_MCP_UNITY_PROJECT` set when a Unity project is available.
...
- Describe this cut as the focused full-node-support foundation release.
- Call out the setup panel, node catalog classification, promoted graph-addable node families, and promoted-node contract replay.
...
- Use post-release feedback to choose the next `1.4.0` slice across explicit configuration-heavy nodes, externally asset-bound node fixtures, deeper subgraph composition, and remaining verified connection routes.
